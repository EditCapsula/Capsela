...
export default function CookieBanner() {
  ...
  return (
    <div className="absolute left-0 right-0 bottom-0 z-40 px-4 pb-[calc(env(safe-area-inset-bottom)+14px)] pt-4 bg-card border-t border-border">
      <div className="text-[12px] text-ink leading-[1.45]">Mesure d&apos;audience</div>
      <div className="text-[11px] text-muted leading-[1.45] mt-[3px]">
      ...
      <div className="flex items-center gap-[9px] mt-[13px]">
        <button
          ...
          className="flex-1 h-[42px] rounded-full border border-border bg-cream text-[12px] text-ink cursor-pointer"
        >
          Refuser
        </button>
        <button
          onClick={() => choose("granted")}
          className="flex-1 h-[42px] rounded-full bg-terracotta text-[12px] text-cream cursor-pointer"
        >
          Accepter
        </button>
      </div>
